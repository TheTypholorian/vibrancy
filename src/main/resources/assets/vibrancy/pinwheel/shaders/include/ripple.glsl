...
vec4 rippleEnchant(float _GameTime, float _dist, vec2 _FragTexCoord) {
    vec4 min = vec4(0.17, 0.07, 0.3, 1);
    vec4 max = vec4(0.64, 0.32, 1, 1);
    return ripple(min, max, _GameTime, _dist, 4, 3, 0.5, sqrt(clamp(distance(_FragTexCoord, vec2(0.5, 0.5)) / 0.7, 0, 1)));
}
